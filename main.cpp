 ...
 * Slot as many as customers into the hotel.Slot as many as customers into the hotel.
 */
#include <iostream>
#include <fstream>
#include "Room.h"
...
void checkOut(Hotel &myHotel, Date &currDate) {
    for (unsigned int i = 0; i < myHotel.getOccupiedBedR().size(); i++) {
        ...
        if (temp.getOutDate() == currDate) { //if the check-out date of the occupant match the current date, check-out
            temp.checkOut();
            myHotel.getOccupiedBedR().erase(myHotel.getOccupiedBedR().begin() + i);//remove the room from the occupied vector
...
void roomArrange(Hotel &myHotel, Queue<Customer> &customers, int nBedNeed) {
    ...
    if (nBedNeed == 1) {
        ...
        if (!myHotel.getUnoccupied2BedR().isEmpty()) { //case: 2-bed room is available
            ...
            takeIn(myHotel, customer, room);
            myHotel.addNumServed(1);
        } else if (myHotel.getUnoccupied1BedR().getSize() >=2) {//case: 1-bed room is available and have 2 rooms available
        ...
        if (!myHotel.getUnoccupied3BedR().isEmpty()) { //case: 3-bed room is available
            ...
            takeIn(myHotel, customer, room);
            myHotel.addNumServed(1);
        } else if (myHotel.getUnoccupied2BedR().getSize() >= 1 && myHotel.getUnoccupied1BedR().getSize() >=2) { //case: 2-bed room and 1-bed room are available and have 1 (2bed) and 1 (1bed) rooms available
            Room room1 = myHotel.getUnoccupied2BedR().dequeue();
            Room room2 = myHotel.getUnoccupied1BedR().dequeue();
            takeIn(myHotel, customer, room1);
            takeIn(myHotel, customer, room2);
            myHotel.addNumServed(1);
        } else if (myHotel.getUnoccupied2BedR().getSize() >=2) { //case: 2-bed room is available and have 2 rooms available
            Room room1 = myHotel.getUnoccupied2BedR().dequeue();
            Room room2 = myHotel.getUnoccupied2BedR().dequeue();
            takeIn(myHotel, customer, room1);
            takeIn(myHotel, customer, room2);
            myHotel.addNumServed(1);
        } else if (myHotel.getUnoccupied1BedR().getSize() >=3) { //case: 1-bed room is available and have 3 rooms available
            Room room1 = myHotel.getUnoccupied1BedR().dequeue();
            Room room2 = myHotel.getUnoccupied1BedR().dequeue();
...
void report(const Hotel &myHotel) {
    std::cout << std::endl;
    std::cout << "The total number of customers with a unique ID have been served is " << myHotel.getNumServed()
              << std::endl;
}
